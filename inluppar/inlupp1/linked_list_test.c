...
{
  ioopm_list_t *ll = ioopm_linked_list_create();
  ioopm_linked_list_append(ll, 42);
...
{
  ioopm_list_t *ll = ioopm_linked_list_create();
  ioopm_linked_list_prepend(ll, 42);
  CU_ASSERT_EQUAL(ioopm_linked_list_get(ll, 0), 42);
  CU_ASSERT_EQUAL(ioopm_linked_list_length(ll), 1);
  ioopm_linked_list_destroy(ll);
}
...
void test_prepend_multiple_entries()
{
  ...
  for (int i = 0; i < 10; i++)
  {
    ...
    CU_ASSERT_EQUAL(ioopm_linked_list_get(ll, 0), list_entries[i]);
  }
  CU_ASSERT_EQUAL(ioopm_linked_list_length(ll), 10);
  for (int i = 0; i < 10; i++)
  {
    int idx = 9 - i;
    CU_ASSERT_EQUAL(ioopm_linked_list_get(ll, i), list_entries[idx]);
  }
  ioopm_linked_list_destroy(ll);
}
...
void test_get_first_element()
{
  ...
  CU_ASSERT_EQUAL(ioopm_linked_list_get(ls, 0), list_entries[0])
...
{
  ioopm_list_t *ls = make_test_list(5, 0);
  CU_ASSERT_EQUAL(ioopm_linked_list_get(ls, 2), list_entries[2])
  ioopm_linked_list_destroy(ls);
}
...
{
  ioopm_list_t *ls = make_test_list(5, 0);
  CU_ASSERT_EQUAL(ioopm_linked_list_get(ls, 4), list_entries[4])
  ioopm_linked_list_destroy(ls);
}
...
{
  ioopm_list_t *ll = make_test_list(0, 0);
  CU_ASSERT_FALSE(ioopm_linked_list_contains(ll, 42))
  ioopm_linked_list_destroy(ll);
}
...
{
  ioopm_list_t *ll = make_test_list(1, 0);
  CU_ASSERT(ioopm_linked_list_contains(ll, list_entries[0]))
  CU_ASSERT_FALSE(ioopm_linked_list_contains(ll, list_entries[1]))
  ioopm_linked_list_destroy(ll);
}
...
void test_contains_multiple()
{
  ...
  for (int i = 0; i < 16; i++)
  {
    CU_ASSERT(ioopm_linked_list_contains(ll, list_entries[i]))
    CU_ASSERT_FALSE(ioopm_linked_list_contains(ll, list_entries[16+i]))
  }
  ioopm_linked_list_destroy(ll);
}
