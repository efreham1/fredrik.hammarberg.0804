...
bool compare_eq_value(elem_t a, elem_t b)
{
  return strcmp(a.ptr_v, b.ptr_v) == 0;
}

bool compare_eq_keys(elem_t a, elem_t b)
{
  return a.int_v == b.int_v;
}

bool compare_lt_keys(elem_t a, elem_t b)
{
  return a.int_v < b.int_v;
}
...
{
  ioopm_hash_table_t *ht = ioopm_hash_table_create(simple_hash_int, compare_eq_keys, compare_eq_value, compare_lt_keys);
...
void test_keys_different_buckets()
{
  ...
  for (int i = 0; i < 3; i++)
  {
    ...
    int j = 0;
    while (compare_eq_keys(key, (elem_t) {.int_v = keys[j]}) == false)
    {
      j += 1;
      if (j == 3)
      {
...
void test_keys_same_bucket()
{
  ...
  for (int i = 0; i<4; i++)
  {
    for (int j = 0; j<4; j++)
    {
      if (compare_eq_keys((elem_t) {.int_v = keys[j]}, ioopm_linked_list_get(keys_list, i)))
      {
        found[j] = true;
        break;
      }
